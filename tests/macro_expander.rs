...
fn parse_and_expand(source: &str) -> (Vec<Rc<RefCell<Statement>>>, Rc<RefCell<TrussDiagnosticEngine>>) {
...
fn test_expand_simple_macro() {
    ...
    assert!(matches!(&*stmts[1].borrow(), Statement::ExpressionStatement { .. }));
}

#[test]
fn test_expand_macro_multiple_args() {
    let (stmts, _) = parse_and_expand(
    ...
    if let Statement::FunctionDecl { body, .. } = &*stmts[1].borrow() {
        ...
        match &*body_ref {
            truss::ast::statement::FunctionBody::Expression(expr) => {
                assert!(!matches!(&*expr.borrow(), truss::ast::expression::Expression::MacroInvocation { .. }));
            }
            truss::ast::statement::FunctionBody::Statements(inner) => {
                assert!(!inner.is_empty());
                if let Statement::ExpressionStatement { expression } = &*inner[0].borrow() {
                    assert!(!matches!(&*expression.borrow(), truss::ast::expression::Expression::MacroInvocation { .. }));
                }
            }
...
fn test_macro_symbold_declared() {
    ...
    let scope = krate.borrow().modules.get("test").unwrap().borrow().scope.clone().unwrap();
    let symbol = scope.borrow().get_symbol("id");
    assert!(symbol.is_some());
    if let Some(sym) = symbol {
        assert!(matches!(&*sym.borrow(), truss::symbol::Symbol::Macro { .. }));
    }
}
